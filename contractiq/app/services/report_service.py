...
def contract_summary(db: Session, from_date: date | None = None, to_date: date | None = None) -> dict[str, Any]:
    query = db.query(Contract)
    if from_date: query = query.filter(Contract.end_date >= from_date)
    if to_date: query = query.filter(Contract.start_date <= to_date)
    contracts = query.all()
...
def obligation_summary(db: Session, from_date: date | None = None, to_date: date | None = None) -> dict[str, Any]:
    query = db.query(Obligation)
    if from_date: query = query.filter(Obligation.due_date >= from_date)
    if to_date: query = query.filter(Obligation.due_date <= to_date)
...
def compliance_summary(db: Session, from_date: date | None = None, to_date: date | None = None) -> dict[str, Any]:
    query = db.query(Contract)
    if from_date: query = query.filter(Contract.end_date >= from_date)
    if to_date: query = query.filter(Contract.start_date <= to_date)
    contracts = query.all()
