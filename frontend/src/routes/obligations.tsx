...
function ObligationsPage() {
  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 px-5 py-8 lg:px-8">
    ...
    <section className="overflow-x-auto rounded-lg bg-card shadow-hairline"><table className="w-full min-w-[920px] text-left text-sm"><thead><tr className="border-b border-border text-xs text-muted-foreground"><th className="px-4 py-3">Obligation</th><th className="px-4 py-3">Due</th><th className="px-4 py-3">Assigned</th><th className="px-4 py-3">Priority</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Actions</th></tr></thead><tbody>{(list.data ?? []).map((item) => { const days = daysUntil(item.due_date); const owner = userList.data?.find((user) => user.id === item.assigned_to); return <tr key={item.id} className="border-b border-border/60 last:border-0"><td className="px-4 py-3"><p className="font-medium">{item.title}</p><p className="text-xs text-muted-foreground">{item.obligation_type}</p></td><td className={`px-4 py-3 ${days < 0 ? "text-destructive" : days <= 3 ? "text-amber-600" : ""}`}>{days < 0 ? `${Math.abs(days)} days overdue` : days === 0 ? "Due today" : days === 1 ? "Due tomorrow" : `Due in ${days} days`}</td><td className="px-4 py-3">{owner?.full_name ?? "Unassigned"}</td><td className="px-4 py-3">{item.priority ?? "Medium"}</td><td className="px-4 py-3">{item.status}</td><td className="px-4 py-3"><div className="flex gap-1"><Button size="icon" variant="ghost" title="Edit" onClick={() => startEdit(item)}><Pencil className="size-4" /></Button>{["Pending", "In Progress", "Delayed", "Overdue"].includes(item.status) ? <Button size="icon" variant="ghost" title="Complete" onClick={() => act(obligations.status(item.id, "Completed"), "Obligation completed", item.contract_id)}><Check className="size-4 text-jade" /></Button> : null}<Button size="icon" variant="ghost" title="Delete" onClick={() => act(obligations.delete(item.id), "Obligation deleted", item.contract_id)}><Trash2 className="size-4 text-destructive" /></Button></div></td></tr>; })}</tbody></table>{!list.isLoading && !list.data?.length ? <p className="p-10 text-center text-sm text-muted-foreground">No obligations yet.</p> : null}</section>
  </main>;
}
