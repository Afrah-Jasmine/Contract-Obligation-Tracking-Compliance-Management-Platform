...
@Component({
 ...
 <ng-container *ngIf="data() as d">
   <section class="stats">
     <a routerLink="/contracts"><mat-card><span>Total contracts</span><strong>{{d.contracts.total}}</strong><small>{{pct(d.contracts.active,d.contracts.total)}}% active</small></mat-card></a>
     <a routerLink="/contracts"><mat-card><span>Active contracts</span><strong>{{d.contracts.active}}</strong><small>Currently active</small></mat-card></a>
     <a routerLink="/contracts"><mat-card class="warning"><span>Expired contracts</span><strong>{{d.contracts.expired}}</strong><small>Requires attention</small></mat-card></a>
     <a routerLink="/obligations"><mat-card><span>Pending obligations</span><strong>{{d.obligations.pending + d.obligations.in_progress}}</strong><small>{{d.obligations.in_progress}} in progress</small></mat-card></a>
     <a routerLink="/obligations"><mat-card class="risk"><span>Overdue obligations</span><strong>{{d.obligations.overdue}}</strong><small>Needs immediate action</small></mat-card></a>
     ...
     <a routerLink="/compliance"><mat-card><span>Compliant contracts</span><strong>{{d.compliance.compliant}}</strong><small>{{d.compliance.non_compliant}} non-compliant</small></mat-card></a>
   </section>
   <section class="grid">
 ...
 `,
 styles:[`
 .stats{display:grid;grid-template-columns:repeat(7,1fr);gap:14px;margin-bottom:16px}.stats a{text-decoration:none;color:inherit}.stats mat-card{height:100%;border-left:4px solid var(--seal);padding:16px}.stats .risk{border-left-color:var(--rose)}.stats .warning{border-left-color:#b9852b}
 ...
 @media(max-width:1200px){.stats{grid-template-columns:repeat(4,1fr)}}@media(max-width:900px){.stats{grid-template-columns:repeat(2,1fr)}}@media(max-width:800px){.grid{grid-template-columns:1fr}.stats{grid-template-columns:1fr 1fr}}@media(max-width:520px){.stats{grid-template-columns:1fr}}
 `]
})
